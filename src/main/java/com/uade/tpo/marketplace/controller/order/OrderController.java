...
public class OrderController {
    ...
    @GetMapping("/me")
    public ResponseEntity<java.util.List<OrderResponse>> getMyOrders(Authentication auth) {
        String email = auth.getName();
        User user = userRepository.findByEmail(email)
                .orElseThrow(() -> new IllegalStateException("Usuario no encontrado"));

        java.util.List<OrderResponse> orders = orderService.getOrdersForUser(user);
